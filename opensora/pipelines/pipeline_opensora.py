...
class OpenSoraPipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, num_frames, height, width, dtype, device, generator, latents=None):
        shape = (
            ...
            (math.ceil((int(num_frames) - 1) / 4) + 1) if int(num_frames) % 2 == 1 else math.ceil(int(num_frames) / 4), 
            math.ceil(int(height) / 8),
            math.ceil(int(width) / 8),
        )
    ...
    def __call__(
        ...
        use_casual_frame_mask: bool = False,
        **kwargs,
        ...
        if use_casual_frame_mask:
            block_size = 240*320//8//8//4
            num_blocks = 8
            small_indices = torch.arange(num_blocks).view(-1, 1)  # shape: (8, 1)
            small_mask = (small_indices <= small_indices.T).int()  # shape: (8, 8)
            attention_mask = small_mask.repeat_interleave(block_size, dim=0).repeat_interleave(block_size, dim=1)
            attention_mask = attention_mask * 0 + (1 - attention_mask) * -10000
            attention_mask_casual = attention_mask.to(latents.device, dtype=latents.dtype)
        else:
            attention_mask_casual = None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    ...
                    attention_mask_casual=attention_mask_casual,
