...
class OpenSoraT2V(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        attention_mask_casual: Optional[torch.Tensor] = None,
        ...
        for block_idx, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                if hidden_states_vid is not None:
                    if attention_mask_casual is not None:
                        attention_mask_vid = attention_mask_casual
                        # print(f"using mask: {attention_mask_vid.shape}")
